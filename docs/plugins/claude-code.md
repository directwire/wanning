...
> W-41 落地(证据全取自 W-19 真插实测,2026-09-02)。取证在档
> (W-19 两节:探路轮 + 根因修正)。
...
| 项 | 状态 |
|---|---|
| 端到端复测 | 机制已实证;**复测烧 Claude 模型额度**(W-19 两轮合计 ≈$2.38,记账在档),重跑由所有者拍板 |
| claude 升级后的行为差异 | 基线 2.1.234;新版本 `.mcp.json` 审批/挂载行为如有变化,待所有者环境重测 |
