...
//!
//! TODO(账户开通后)清单(见各处注释;W-50 公开面复核后的收窄版):
//! 1. 请求签名(app_key/secret 的签名算法)与 access_token 的获取/续期——
//!    **公开面查不到**(W-50 直核:VOP API 文档面 11 组全枚举无签名文档,
//!    SDK 示例只传 appSecret 进 DefaultJdClient,算法封在 SDK 内部),保持占位;
//!    解锁路径:账户开通后,按调研在档的「开户后人工核清单」逐项核对。
//! 2. 真实网关 URL(SDK 示例 SERVER_URL 是占位常量,公开面无真值)。
//! 3. search/create_order 的真实请求/响应字段映射——接口名公开面可枚举
//!    (搜索商品 5833/提交预占订单 5856/确认订单 5839 等,见调研文档),
//!    字段树开户后按现物回填。
//! 4. 错误码语义映射——**码表已公开面直核**(2002 合同权限不足/2001 用户权限
//!    不足/2010 请求过于频繁/2011 请求接口受限等,见调研文档),实测语义后钉测试。
//!
//! 对接形态注意(W-50 直核):京东 VOP 支付 = 企业采购付款(收银台表单提交/
//! 对公转账/结算流水号),**没有**「先签约后服务端扣款」的免密代扣形态——
//! 与支付宝「协议内扣款」(alipay.rs,W-50 已填实)不同构;本模块的闸管意图/
//! 渠道管资金的分层不变。
