...
//! wanning-demo --anchor-sign <wal> --key <key.hex> --out <anchor.json>   (老板侧,W-23,v1 HMAC)
//! wanning-demo --anchor-verify <wal> --anchor <anchor.json> --key <key.hex>
//! wanning-demo --anchor-sign-v2 <wal> --seed <seed.hex> --out <anchor.json>  (老板侧,W-31,ed25519)
//! ```
//!
//! `--dry-run` 默认 true(离线);设为 false 走真实消费路径,**先过 fail-closed 护栏**
//! ([`wanning_demo::guard`]):任何一项 env 缺失立即拒绝并打印缺什么。
//! `--export-audit` 把一份审计日志渲染成自包含 HTML 回放页(零 JS 零外链,
//! 先对账后产出,坏账绝不写输出文件)。
//! `--anchor-sign`/`--anchor-verify` 是**老板侧**审计锚点(W-23,v1 HMAC):用
...
use wanning_demo::anchor_cmd;
use wanning_demo::anchor_v2;
use wanning_demo::audit_html;
use wanning_demo::full_loop;
use wanning_demo::guard;
use wanning_demo::scenario;
...
enum Mode {
    ...
    /// 老板侧:拿锚点 + 密钥验当前账本(HMAC 验证需要密钥,见 anchor_cmd 模块注释)。
    AnchorVerify {
        wal: PathBuf,
        anchor: PathBuf,
        key: PathBuf,
    },
    /// 老板侧:ed25519 种子签出锚点 v2(公钥随锚点走,第三方零密钥可验,W-31)。
    AnchorSignV2 {
        wal: PathBuf,
        seed: PathBuf,
        out: PathBuf,
...
/// 老板侧签锚点:读密钥 → 对账 → 签 → 原子落盘(库面 [`anchor_cmd::sign`])。
...
/// 老板侧验锚点:锚点可信(MAC)→ 完整性链 → 前缀逐字段比对(库面
...
/// 老板侧签锚点 v2:读 ed25519 种子 → 对账 → 签 → 原子落盘(库面
...
fn parse_args(args: &[String]) -> Result<Cli, String> {
    ...
    if let Some(wal) = anchor_wal {
        ...
        let key = key_path.ok_or_else(|| {
            "--anchor-sign/--anchor-verify 需要 --key <密钥文件>(64 位十六进制的老板密钥);\
             --help 看用法"
                .to_string()
        })?;
...
fn print_usage() {
    println!(
        "wanning-demo —— Wanning P0 演示(全离线 + 本地 mock,零真实消费)\n\
         ...
         \x20 --anchor-sign <wal>   老板侧:用自己的密钥签出审计锚点(锚住「前 N 行\n\
         \x20                     内容 + 行数 + 链尾」),堵住完整性链的已知边界\n\
         \x20                     (只改尾行/整体截尾本地验不住)。对账先行,坏账不签。\n\
         \x20 --anchor-verify <wal> 老板侧:拿锚点验当前账本——截尾/改尾行当场现形;\n\
         \x20                     锚定后合法追加的新行不影响通过(前缀锚语义)。\n\
         \x20 --anchor-sign-v2 <wal> 老板侧(v2,ed25519):用种子签出第三方可验锚点,\n\
